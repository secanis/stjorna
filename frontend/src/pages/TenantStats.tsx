...
import {
  Package, Folder, Image, Users, HardDrive, Clock, BarChart3, RefreshCw, ArrowLeft,
} from 'lucide-solid';
...
export default function TenantStats() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const tenant = await pb.collection('tenants').getOne(params.id).catch(() => null);
    ...
    <div class="space-y-6">
      <div class="flex items-start justify-between">
        <div>
          ...
          <Show when={snapshot()}>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
              tenant id{' '}
              <code class="text-xs text-gray-900 dark:text-gray-100">{snapshot()!.tenant.id}</code>
              {' · '}snapshot taken{' '}
        ...
        <div class="flex gap-2">
          <A
            href={`/tenants/${params.id}`}
            class="inline-flex items-center gap-1 text-sm px-3 py-1.5 rounded border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800"
          >
            Edit tenant
          </A>
          <button
        ...
        <Show when={s.storage.by_mime_type.length > 0}>
          <div class="bg-white dark:bg-gray-800 rounded-lg p-4 mt-4">
                ...
                <div>
                  ...
                  <div class="flex">
                    ...
                    <div
                      ...
                      style={{ width: `${Math.max(0, (m.count / maxMimeCount()) * 100 - (m.bytes / maxMimeBytes()) * 100)}%`, 'min-width': '0' }}
                      title={`${m.count} files`}
                    />
                  </div>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">
              Solid bar = share of total bytes. Lighter bar = share of file count.
            </p>
          </div>
        </Show>
      </section>
