...
import { Package, Folder, Image, Users, HardDrive, Clock, BarChart3, RefreshCw } from 'lucide-solid';
import { authStore } from '~/stores/auth';
import { tenantStore } from '~/stores/tenant';
import { fetchMyStats, fetchTenantStats, type StatsSnapshot, StatsFormatting } from '~/services/stats';
                ...
                <div>
                    ...
                    <Show when={snapshot()}>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                            {snapshot()!.tenant.name} · plan: <span class="font-medium text-gray-900 dark:text-gray-100">{snapshot()!.tenant.plan}</span>
                            {' · '}
                            ...
                            {s.storage.by_mime_type.map((m) => (
                                <div>
                                    ...
                                    <div class="flex gap-1">
                                        ...
                                        <div class="bg-blue-300 dark:bg-blue-700 h-2 rounded-r" style={{ width: `${(m.count / maxMimeCount()) * 100 - (m.bytes / maxMimeBytes()) * 100}%`, 'min-width': '0' }} title={`${m.count} files`} />
                                    </div>
                                </div>
                            ))}
                        </div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">Solid bar = share of total bytes. Striped bar = share of file count.</p>
                    </div>
                </Show>
            </section>
