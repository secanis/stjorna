...
export default function TenantSelector() {
  ...
  return (
    <div class="relative group">
      <div class="flex items-center gap-2 cursor-pointer px-3 py-1.5 rounded hover:bg-gray-700 transition-colors border border-gray-600 bg-gray-700">
      ...
      <div class="absolute left-0 mt-1 w-64 bg-gray-800 rounded-lg shadow-xl border border-gray-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
        <div class="py-1">
          <Show when={authStore.isPBAdmin}>
            <div class="px-3 py-2 border-b border-gray-700">
              <div class="text-xs text-gray-500 uppercase">System</div>
            </div>
            <button
              onClick={() => authStore.switchTenant(null)}
              class={`w-full text-left px-4 py-2 text-sm flex items-center justify-between ${
                currentValue() === '' ? 'text-blue-400 bg-gray-700' : 'text-gray-300 hover:bg-gray-700 hover:text-white'
              }`}
            >
            ...
            <div class="px-3 py-2 border-b border-gray-700 mt-1">
              <div class="text-xs text-gray-500 uppercase">Tenants</div>
            </div>
          </Show>
          ...
          <For each={authStore.tenants}>
            {(t) => (
              <button
                ...
                class={`w-full text-left px-4 py-2 text-sm flex items-center justify-between ${
                  currentValue() === t.tenant ? 'text-blue-400 bg-gray-700' : 'text-gray-300 hover:bg-gray-700 hover:text-white'
                }`}
              >
          ...
          <Show when={authStore.tenants.length === 0 && !authStore.isPBAdmin}>
            <div class="px-4 py-3 text-gray-500 text-sm">No tenants available</div>
          </Show>
        </div>
      </div>
    </div>
