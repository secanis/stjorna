        ...
        <div class="min-h-screen flex items-start justify-center p-6">
            <div class="w-full max-w-xl space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <button onClick={toggleTheme} class="text-sm px-3 py-1 rounded border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Toggle theme">
                ...
                <form onSubmit={save} class="space-y-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-5">
                    ...
                    <div>
                        ...
                        />
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            STJÓRNA API keys (<code>stjorna_…</code>) are auto-exchanged for a STJÓRN A service-user JWT on save, so they unlock <code>/api/collections/*</code> like any other STJÓRN A user. User JWTs and PB admin tokens are stored verbatim.
                        </p>
                    ...
                    <div class="flex items-center gap-3">
                        <button type="submit" disabled={saving()} class="px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-50">
                            {saving() ? 'Saving…' : 'Save & open catalog'}
                        </button>
                        <button type="button" onClick={testConnection} disabled={testing()} class="px-4 py-2 rounded border border-gray-300 dark:border-gray-700 text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50">
                            {testing() ? 'Testing…' : 'Test connection'}
                        </button>
                    </div>

                    <Show when={saveError()}>
                        <div class="text-sm px-3 py-2 rounded bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-300">{saveError()}</div>
                    </Show>

                    <Show when={testResult()}>
                        <div class={`text-sm px-3 py-2 rounded ${testResult()!.ok ? 'bg-green-50 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-300'}`}>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    Hint: Set the URL first, save, the app routes to <code>/</code>. Open the API log at the bottom of every page to watch STJÓRNA traffic live.
                </p>
            </div>
        </div>
    );
