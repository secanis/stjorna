 ...
 * The MailerRecordEvent exposes the raw OTP as e.meta.password. This hook uses
 * that value first because it is exact and independent of the email template.
 * As a fallback it parses the rendered HTML/text body for a <strong>12345678
 * </strong> style value, and finally for any digit sequence whose length matches
 * the collection's configured OTP length.
 */
...
if (OTP_LOG_ENABLED) {
  onMailerRecordOTPSend((e) => {
    try {
      ...
      // The OTP code is available directly in the event meta. Prefer that
      // over parsing the rendered email body because it is always exact and
      // works regardless of the email template format.
      let code = "";
      try {
        if (e.meta && e.meta.password) {
          code = String(e.meta.password);
        }
      } catch (_) {}

      // Fallback: parse the rendered email body.
      if (!code) {
        ...
        if (!code) {
          console.log("[stjorna-otp-logger] warning: could not extract OTP from email body");
        }
      }
      ...
      try {
        if (e.record) {
          if (typeof e.record.Email === "function") {
            email = e.record.Email();
          } else if (typeof e.record.email === "function") {
            email = e.record.email();
          } else if (typeof e.record.get === "function") {
            email = e.record.get("email") || "unknown";
          }
        }
      } catch (_) {}
