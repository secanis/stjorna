...
module.exports = (router) => {
    router
        ...
        .get((req, res) => {
            ...
            if (products) {
                trackingHelper.apiTrack(req, 'list products', {
                    c_n: 'products',
                    c_t: req.originalUrl,
                });
                res.send(products);
        ...
        .put((req, res) => {
            ...
            logger.log(
                'debug',
                `product - add product with ID: ${newItem._id}`
            );
    ...
    router
         ...
         */
        .get((req, res) => {
            let product = dbHelper.db
                .get('products')
                .find({ _id: req.params.id })
                .value();
            logger.log(
                'debug',
                `product - get product with ID: ${req.params.id}`
            );
            if (product) {
                trackingHelper.apiTrack(req, 'get product', {
                    c_n: 'product',
                    c_p: product.name,
                    c_t: req.originalUrl,
                });
                ...
                logger.error(
                    `error occured: could not load product ${req.params.id}`
                );
                res.status(400).send({
                    message: `Couldn't load product ${req.params.id}`,
                    status: 'error',
                });
            }
        })
        ...
        .post((req, res) => {
            ...
            logger.log(
                'debug',
                `product - update product with ID: ${req.params.id}`
            );
            dbHelper.db
                .get('products')
                .find({ _id: req.params.id })
                ...
                .then(() => {
                    ...
                    if (item && item.updated === newItem.updated) {
                        ...
                        logger.error(
                            `error occured: could not update product '${req.params.id}'`
                        );
                        res.status(400).send({
                            message: `Couldn't update product '${req.params.id}'`,
                            status: 'error',
                        });
                    }
                });
        ...
        .delete((req, res) => {
            logger.log(
                'debug',
                `product - delete product with ID: ${req.params.id}`
            );
            dbHelper.db
                .get('products')
                ...
                .then(() => {
                    ...
                    if (!item) {
                        ...
                        logger.error(
                            `error occured: could not remove products '${req.params.id}'`
                        );
                        res.status(400).send({
                            message: `Couldn't remove products '${req.params.id}'`,
                            status: 'error',
                        });
                    }
                });
