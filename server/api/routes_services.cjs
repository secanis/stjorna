...
module.exports = (router) => {
    router
        ...
        .get((req, res) => {
            ...
            if (services) {
                trackingHelper.apiTrack(req, 'list services', {
                    c_n: 'services',
                    c_t: req.originalUrl,
                });
                res.send(services);
    ...
    router
         ...
         */
        .get((req, res) => {
            let service = dbHelper.db
                .get('services')
                .find({ _id: req.params.id })
                .value();
            if (service) {
                trackingHelper.apiTrack(req, 'get service', {
                    c_n: 'service',
                    c_p: service.name,
                    c_t: req.originalUrl,
                });
                ...
                logger.error(
                    `error occured: couldn't load service ${req.params.id}`
                );
                res.status(400).send({
                    message: `Couldn't load service ${req.params.id}`,
                    status: 'error',
                });
            }
        })
            ...
            dbHelper.db
                ...
                .then(() => {
                    ...
                    if (item && item.updated === newItem.updated) {
                        ...
                        logger.error(
                            `error occured: couldn't update service '${req.params.id}'`
                        );
                        res.status(400).send({
                            message: `Couldn't update service '${req.params.id}'`,
                            status: 'error',
                        });
                    }
                });
            ...
            dbHelper.db
                ...
                .then(() => {
                    ...
                    if (!item) {
                        ...
                        logger.error(
                            `error occured: couldn't remove service '${req.params.id}'`
                        );
                        res.status(400).send({
                            message: `Couldn't remove service '${req.params.id}'`,
                            status: 'error',
                        });
                    }
                });
